...
(defun get (key &key bucket (server "localhost:8098"))
  (let ((request-url (concatenate 'string "http://" server "/riak/" 
                                  ...
                                  (url-encode key :utf-8))))
    (multiple-value-bind 
      (response status headers) (drakma:http-request request-url)
...
(defun delete (key &key bucket (server "localhost:8098"))
  (let ((request-url (concatenate 'string "http://" server "/riak/" 
                                  ...
                                  (url-encode key :utf-8))))
    (multiple-value-bind (response status headers)
	    (drakma:http-request request-url :method :delete)
      (if (or (= status 204) (= status 404)) t nil))))

(defun set (key value &key bucket (server "localhost:8098") 
                           (content-type "text/plain") vclock)
  (let ((request-url (concatenate 'string "http://" server "/riak/" 
                                  (url-encode bucket :utf-8) 
				                          (if key 
                                    (concatenate 
                                      'string "/" (url-encode key :utf-8))
                                    ""))))
    (multiple-value-bind (response status headers)
	    (drakma:http-request request-url
